...
namespace Elgato.StreamdeckSDK.Types.Messages
{
    public abstract class ESDMessage
    {
        ...
        public static ESDMessage Parse(Memory<byte> memory)
        {
            ...
            if (!Enum.TryParse(eventElement.GetString(), true, out ESDEvent streamdeckEvent))
            {
                throw new ESDInvalidEventException(json);
            }
            ...
            if (!(deserializer.Invoke(null, new object?[]{json, JsonOptions}) is ESDMessage message))
            {
